...
If the choice is `4` (divide), check if the second number is zero. If yes, display an error and terminate.

#### Step 8.2:

If the choice is invalid, display an error and terminate.
...
Input employee name (allow spaces).
...
(Optional): Print the entered details for verification.
...
Compare employee salary with `high`.

#### Step 8.2:

If the salary is greater than `high`, update `high` with this salary.
...
If employee salary equals `high`, print employee number, name, and salary.
